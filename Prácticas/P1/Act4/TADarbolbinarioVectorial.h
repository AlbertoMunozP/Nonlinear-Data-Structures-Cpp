...
//IMPLEMENTACION VECTORIAL DEL TAD ARBOL BINARIO
...
class Abin {
	public:
		...
		int profundidad_rec(nodo n) const;
...
template <typename T>
int Abin<T>::profundidad_rec(nodo n) const{
	int p = 0;
	...
	if (p > 0)
		return p-1;
	else // La profundidad de un nodo nulo pongo 0 
		return 0;
}
...
int Abin<T>::altura(nodo n) const{
	if(n == Abin<T>::NODO_NULO)
		...
		return 1+std::max(altura(hijoDrcho(n)),altura(hijoIzqdo(n)));
}
